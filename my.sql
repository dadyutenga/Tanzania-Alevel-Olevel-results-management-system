...
-- Trigger to enforce exam-class-combination and subject validity
DELIMITER //
CREATE TRIGGER `before_insert_tz_alevel_subject_marks`
BEFORE INSERT ON `tz_alevel_subject_marks`
FOR EACH ROW
BEGIN
  DECLARE exam_class_combo_count INT;
  DECLARE max_marks INT;
  DECLARE subject_valid INT;

  -- Check if the exam is allocated to the class and combination
  SELECT COUNT(*)
  INTO exam_class_combo_count
  FROM `tz_alevel_exam_combinations` aec
  WHERE aec.exam_id = NEW.exam_id
    AND aec.class_id = NEW.class_id
    AND aec.session_id = NEW.session_id
    AND aec.combination_id = NEW.combination_id
    AND aec.is_active = 'yes';

  IF exam_class_combo_count = 0 THEN
    SIGNAL SQLSTATE '45000'
    SET MESSAGE_TEXT = 'Exam is not allocated to this class, session, and combination.';
  END IF;

  -- Check if the subject belongs to the combination and is valid for the exam
  SELECT COUNT(*)
  INTO subject_valid
  FROM `tz_alevel_combination_subjects` acs
  JOIN `tz_exam_subjects` es ON acs.subject_name = es.subject_name
  WHERE acs.id = NEW.subject_id
    AND acs.combination_id = NEW.combination_id
    AND es.exam_id = NEW.exam_id
    AND acs.is_active = 'yes';

  IF subject_valid = 0 THEN
    SIGNAL SQLSTATE '45000'
    SET MESSAGE_TEXT = 'Subject does not belong to this combination or exam.';
  END IF;

  -- Validate marks_obtained against max_marks
  SELECT es.max_marks
  INTO max_marks
  FROM `tz_exam_subjects` es
  JOIN `tz_alevel_combination_subjects` acs ON es.subject_name = acs.subject_name
  WHERE acs.id = NEW.subject_id
    AND es.exam_id = NEW.exam_id;

  IF NEW.marks_obtained IS NOT NULL AND (NEW.marks_obtained < 0 OR NEW.marks_obtained > max_marks) THEN
    SIGNAL SQLSTATE '45000'
    SET MESSAGE_TEXT = 'Marks obtained must be between 0 and the maximum marks for the subject.';
  END IF;
END//
DELIMITER ;

-- Trigger for updates to ensure the same validations
DELIMITER //
CREATE TRIGGER `before_update_tz_alevel_subject_marks`
BEFORE UPDATE ON `tz_alevel_subject_marks`
FOR EACH ROW
BEGIN
  DECLARE exam_class_combo_count INT;
  DECLARE max_marks INT;
